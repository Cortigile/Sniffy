...
try {
    ...
    if ($runBuildCheck) {
        ...
        try {
            Invoke-CheckedCommand -FilePath "cmake" -Arguments @("-S", "sniffy", "-B", $buildDir, "-G", $Generator, "-DCMAKE_BUILD_TYPE=Release") -WorkingDirectory $repoRoot
            Invoke-CheckedCommand -FilePath "cmake" -Arguments @("--build", $buildDir, "--config", "Release", "--target", "sniffy") -WorkingDirectory $repoRoot
        }
        catch {
