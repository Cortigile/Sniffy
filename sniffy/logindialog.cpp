    ...
    auth(authenticator)
{
    ...
    registerLink->setText(
        ...
        "<a href=\"https://www.sniffylab.com\" target=\"_blank\">www.sniffylab.com</a>");
    ...
    // Keep account identity stable while authenticated; edit only after logout.
    userEmail->setEnabled(!CustomSettings::hasValidLogin());
    ...
    connect(auth, &Authenticator::authenticationFailed, this, [this](const QString &code, const QString &uiMsg){
        ...
        QString finalMessage = uiMsg;
        if (code == "not_logged-in" || code.contains("Please login on web", Qt::CaseInsensitive)) {
            const QString requestedEmail = userEmail->getText().trimmed();
            if (!requestedEmail.isEmpty()) {
                finalMessage = "Complete web login as " + requestedEmail + ". If another account is logged in browser, log out first.";
            }
        }
        reportFailure(finalMessage, code, false);
    });
...
void LoginDialog::openBrowserForAuth(const QString &email){
    ...
    QUrl startUrl("https://sniffylab.com/scripts/sniffy_auth_start.php");
    QUrlQuery query;
    ...
    info->setName("Complete login in browser as " + email + ". If a different user is already logged in, log out first.");
    info->setColor(Graphics::palette().textLabel);
...
void LoginDialog::finalizeSuccess(const QDateTime &validity, const QByteArray &token, bool authenticationSentManual){
    ...
    if (authenticationSentManual) {
        QString committedEmail = auth ? auth->getPendingEmail().trimmed() : QString();
        if (committedEmail.isEmpty()) {
            committedEmail = userEmail->getText().trimmed();
        }

        if (!committedEmail.isEmpty()) {
            CustomSettings::setUserEmail(committedEmail);
            userEmail->setText(committedEmail);
        }
    }
...
void LoginDialog::open()
{
    ...
    this->raise();
    this->activateWindow();
    const bool hasValidLogin = CustomSettings::hasValidLogin();
    if (logoutLabel) logoutLabel->setVisible(hasValidLogin);
    userEmail->setEnabled(!hasValidLogin);
}
...
void LoginDialog::buttonAction(int isCanceled)
{
    ...
    const QString email = userEmail->getText().trimmed();
    if(email.isEmpty()){
        reportFailure("Email required","empty-email", false);
        return;
    }
