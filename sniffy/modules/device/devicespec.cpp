...
void DeviceSpec::parseSpecification(QByteArray spec){
    ...
    if(specParams.length()>=15){
        ...
        OS_Version = QString::fromUtf8(specParams[8]);
        Protocol_Version_Text = QString::fromUtf8(specParams[9]).trimmed();
        Protocol_Version = FirmwareCompatibility::parseProtocolVersion(Protocol_Version_Text);
        HasCompatibilityMetadata = (Protocol_Version > 0);
        IsProtocolCompatible = true;
        CompatibilityMessage.clear();
