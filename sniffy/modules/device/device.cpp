...
Device::Device(QObject *parent)
{
    ...
    moduleName = "Device";
    moduleIconURI = Graphics::getCommonPath()+"icon_not_connected.png";
...
void Device::updateGUIDeviceList(QList<DeviceDescriptor> deviceList, bool autoConnectSingle){
    ...
    if(deviceWindow->deviceSelection->count()==0){
        ...
        deviceWindow->deviceConnectButton->setDisabledButton(true,0); //disable connect button
...
void Device::disconnectDevice(){
    ...
    setIcon(Graphics::getCommonPath()+"icon_not_connected.png");
...
void Device::passSystemSpecificationToGUI()
{
    ...
    labels.append("FW");
    QString firmwareValue = QStringLiteral("%1 (%2)")
                                .arg(deviceSpec->FW_Version.toHtmlEscaped(),
                                     deviceSpec->Build_Date.toHtmlEscaped());
    if (deviceSpec->HasCompatibilityMetadata && !deviceSpec->IsProtocolCompatible) {
        firmwareValue += QStringLiteral(" <span style=\"color:%1;\">Not Compatible</span>")
                             .arg(Graphics::palette().error.toHtmlEscaped());
    }
    values.append(firmwareValue);
    //labels.append("Free RTOS");
    //values.append(deviceSpec->OS_Version);
    deviceWindow->addModuleDescription("System Info",labels,values);
}

DeviceSpec *Device::getDeviceSpec() const
{
    return deviceSpec;
}
...
void Device::parseData(QByteArray data){
    ...
    if(feature==Commands::CONFIG){
        ...
        setIcon(Graphics::getCommonPath()+"icon_connected.png");
