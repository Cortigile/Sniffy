    ...
    ui(new Ui::DeviceWindow)
{
    ...
    pinoutWidget->setGeometry(ui->widget_device->rect());
        connect(pinoutWidget, &PinoutWidget::togglePinoutRequested,
            this, &DeviceWindow::togglePinoutDisplay);
    ...
    if(watched == ui->widget_device){
        ...
        if(event->type() == QEvent::MouseButtonRelease){
            ...
            if(me->button() == Qt::RightButton)
                return togglePinoutDisplay();
        }
    }
    return QWidget::eventFilter(watched, event);
}

bool DeviceWindow::togglePinoutDisplay()
{
    if(currentDeviceBaseImage.isEmpty())
        return false;
