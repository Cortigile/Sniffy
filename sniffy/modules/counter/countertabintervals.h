#ifndef COUNTERTABINTERVALS_H
#define COUNTERTABINTERVALS_H

#include <QObject>
#include <QVBoxLayout>
#include <QLabel>
#include <QPixmap>

#include "gui/widgetseparator.h"
#include "gui/widgetdial.h"
#include "gui/widgetdialrange.h"
#include "gui/widgetbuttons.h"
#include "gui/widgetswitch.h"

#include "counterdefs.h"
#include "counterconfig.h"

class CounterTabIntervals : public QObject
{
    Q_OBJECT
public:
    explicit CounterTabIntervals(QVBoxLayout *destination, CounterConfig *conf, QWidget *parent = nullptr);

    WidgetButtons *buttonStart;
    WidgetButtons *buttonsEventsSeq;
    WidgetSwitch *switchEdgeEventA;
    WidgetSwitch *switchEdgeEventB;
    WidgetDialRange *dialTimeout;
    QHBoxLayout *horizontalLayout_switchArea;
    QLabel *labelPicEventsSeq;
    QPixmap *pixmapEventsSeq = nullptr;

    void loadGraphics();
    void setStartButton(bool start);
    void enableConfigButtons(bool enable);

private:
    CounterConfig *conf;

    bool seqAB = false, eventA = false, eventB = false;
    int width, height;

    QString PATH_IMG_EDGE_RISING = Graphics::getCommonPath()+"cnt_edge_rising.png";
    QString PATH_IMG_EDGE_FALLING = Graphics::getCommonPath()+"cnt_edge_falling.png";
    QString PATH_IMG_EVENT_AB_RR = Graphics::getCommonPath()+"cnt_ab_rr.png";
    QString PATH_IMG_EVENT_AB_RF = Graphics::getCommonPath()+"cnt_ab_rf.png";
    QString PATH_IMG_EVENT_AB_FR = Graphics::getCommonPath()+"cnt_ab_fr.png";
    QString PATH_IMG_EVENT_AB_FF = Graphics::getCommonPath()+"cnt_ab_ff.png";
    QString PATH_IMG_EVENT_BA_RR = Graphics::getCommonPath()+"cnt_ba_rr.png";
    QString PATH_IMG_EVENT_BA_RF = Graphics::getCommonPath()+"cnt_ba_rf.png";
    QString PATH_IMG_EVENT_BA_FR = Graphics::getCommonPath()+"cnt_ba_fr.png";
    QString PATH_IMG_EVENT_BA_FF = Graphics::getCommonPath()+"cnt_ba_ff.png";

    void setSeqPicture();

private slots:    
    void eventsSequenceChangedCallback(int index);
    void eventAEdgeChangedCallback(int index);
    void eventBEdgeChangedCallback(int index);
};

#endif // COUNTERTABINTERVALS_H
