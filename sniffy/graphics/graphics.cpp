...
static QString boardTemplateId(const QString &boardBaseName){
    QFile boardConfig(QStringLiteral(":/config/") + boardBaseName + QStringLiteral(".json"));
    if(!boardConfig.open(QIODevice::ReadOnly))
        return QString();

    const QJsonDocument doc = QJsonDocument::fromJson(boardConfig.readAll());
    return doc.object().value(QStringLiteral("template")).toString();
}

static QString boardImageFileName(const QString &templateId){
    if(templateId.compare(QStringLiteral("NUCLEO-144"), Qt::CaseInsensitive) == 0)
        return QStringLiteral("nucleo_144.png");
    if(templateId.compare(QStringLiteral("NUCLEO-64"), Qt::CaseInsensitive) == 0)
        return QStringLiteral("nucleo_64.png");
    return QString();
}
...
QString getBoardImage(const QString &boardBaseName){
    ...
    const QString boardImageName = boardImageFileName(boardTemplateId(boardBaseName));
    if(!boardImageName.isEmpty()){
        const QString candidate = getCommonPath() + boardImageName;
